...
public class Cmd_getHeading extends RemoteCommand
{
...
public Cmd_getHeading(int command) 
{
	this();
	this.setId(command);
}
...
{
	return(Cmd_getHeading.description);
}
...
public int getIndex()
{
	return((( RemoteParameterUint8) this.get(Cmd_getHeading.INDEX_SENSOR)).getValue());
}

public int getRange()
{
	return((( RemoteParameterUint8) this.get(Cmd_getHeading.INDEX_RANGE)).getValue());
}


public int getGain()
{
	return((( RemoteParameterUint8) this.get(Cmd_getHeading.INDEX_GAIN)).getValue());
}


public int getAddress()
{
	return((( RemoteParameterUint8) this.get(Cmd_getHeading.INDEX_ADDRESS)).getValue());
}



public static Cmd_getHeading getCommand(int id)
{
	Cmd_getHeading cmd;
	cmd = new Cmd_getHeading(id);
...
public static Cmd_getHeading getCommand(int command, float heading)
{
	
	Cmd_getHeading cmd;
	cmd = Cmd_getHeading.getCommand(command);
	cmd.setData(heading);
